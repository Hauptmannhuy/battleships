const gamestate = { moveOrder: false, player1: null, player2: null };
...
function playerStrike(target, innerBoard) {
  let coord = target
    ...
    .split(" ")
    .map((n) => n * 1);
  innerBoard.receiveStrike(coord);
  gamestate["moveOrder"] = true;
}

function computerStrike(board) {
  board.receiveStrike(getRandomCoord(board.missedShots.concat(board.hitShots)));
